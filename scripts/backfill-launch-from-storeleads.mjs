...
try {
  ...
  if (!APPLY) { console.log("\nDRY RUN — re-run with --apply to write."); }
    ...
    // date_trunc → first of month = month/year precision; tag provenance so the
    // source/precision is explicit (own-sourced earliest_product stays day-precise).
    const res = await sql`
    ...
    console.log(`✓ Applied: filled ${res.count}.  launched_at now: ${after.la}/${after.tot} (${Math.round(100 * after.la / after.tot)}%).`);
  }
