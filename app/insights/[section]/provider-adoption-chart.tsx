...
type Series = { years: number[]; startYear: number; baselineYear: number; providers: Provider[]; storesCumulative: number[] };
...
export function ProviderAdoptionChart({ country, platform }: { country: string; platform: string }) {
  ...
  const [metric, setMetric] = useState<"count" | "share">("count"); // absolute merchants vs market penetration %
  ...
  const stores = data?.storesCumulative ?? [];
  ...
  // value at year i: raw cumulative count, or that provider's PENETRATION (% of stores launched by
  // year i that carry it). Provider counts can overlap (multi-gateway stores), so shares needn't sum
  // to 100 — each line is "what % of that cohort uses this provider".
  const valAt = (p: Provider, i: number) => (share ? (stores[i] ? (100 * p.cumulative[i]) / stores[i] : 0) : p.cumulative[i]);
  const fmt = (v: number) => (share ? `${v.toFixed(v < 10 ? 1 : 0)}%` : Math.round(v).toLocaleString());
  const W = 760, H = 300, padL = 44, padR = 96, padB = 34, padT = 16;
  const iw = W - padL - padR, ih = H - padT - padB;
  const maxY = Math.max(share ? 1 : 1, ...provs.flatMap((p) => years.map((_, i) => valAt(p, i))));
  const xAt = (i: number) => padL + (years.length > 1 ? (i / (years.length - 1)) * iw : 0);
  const yAt = (v: number) => padT + ih - (v / maxY) * ih;
  const path = (p: Provider) => years.map((_, i) => `${i === 0 ? "M" : "L"}${xAt(i).toFixed(1)},${yAt(valAt(p, i)).toFixed(1)}`).join(" ");
  const yTicks = 4;
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-cream/45">
            {share
              ? "Market penetration per provider — % of each launch cohort that carries it (a store can use several)."
              : "Cumulative merchants per provider, credited to each store’s launch year (best estimate of when they chose it)."}
          </p>
        </div>
        <div className="flex flex-col items-end gap-2">
          <div className="flex gap-1 rounded-full border border-cream/12 p-1" title="show absolute merchant counts or each provider's share of the market">
            {([["count", "Count"], ["share", "Share %"]] as const).map(([k, l]) => (
              <button key={k} onClick={() => setMetric(k)}
                className={`rounded-full px-3 py-1 text-xs transition ${metric === k ? "bg-cyan font-semibold text-cyan-deep" : "text-cream/50 hover:text-cream"}`}>{l}</button>
            ))}
          </div>
          ...
          <>
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full" onMouseMove={onMove}>
              ...
              {Array.from({ length: yTicks + 1 }, (_, k) => {
                ...
                return (
                  <g key={k}>
                    ...
                    <text x={padL - 6} y={y + 3} textAnchor="end" fill="var(--color-cream)" fillOpacity="0.4" fontSize="10">{fmt(v)}</text>
                  </g>
                );
              })}
              {/* x labels */}
              ...
              {provs.map((p, i) => {
                ...
                return (
                  <g key={p.label}>
                    <path d={path(p)} fill="none" stroke={c} strokeWidth="2" strokeLinejoin="round"
                      opacity={hover == null ? 0.95 : 0.5} />
                    <circle cx={xAt(last)} cy={yAt(valAt(p, last))} r="3" fill={c} opacity={hover == null ? 1 : 0.5} />
                    {hover != null && <circle cx={xAt(hover)} cy={yAt(valAt(p, hover))} r="3.5" fill={c} />}
                  </g>
                );
              })}
            </svg>
            {hover != null && data && (
              <div className="pointer-events-none absolute z-10 -translate-x-1/2 rounded-lg border border-cream/20 bg-ink-deep/95 px-3 py-2 text-xs shadow-xl"
                ...
                {[...provs].sort((a, b) => valAt(b, hover) - valAt(a, hover)).map((p) => (
                  <div key={p.label} className="mt-0.5 flex items-center gap-1.5">
                    ...
                    <span className="ml-auto pl-3 font-medium text-cream tabular-nums">{fmt(valAt(p, hover))}</span>
                  </div>
                ))}
              </div>
            )}
