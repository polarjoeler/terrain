...
export type ServicePartner = {
  ...
  websiteUrl: string | null; storesLinked: number | null;
};
  ...
  try {
    const rows = await sql`
      SELECT id, name, type, country, location, bio, services, skills, partner_programs,
             ...
             is_pro, website_url, stores_linked
      FROM service_partners
      ORDER BY is_pro DESC NULLS LAST, rating DESC NULLS LAST, completed_projects DESC NULLS LAST, name`;
    return rows.map((r) => ({
