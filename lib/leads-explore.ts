...
export async function exploreLeads(limit = 25000): Promise<ExploreLead[]> {
  const rows = await db()<{
    ...
    estimated_monthly_sales: string | null; plus: boolean; email: string | null;
    ...
    SELECT domain, name, category, country, city, estimated_monthly_sales, plus, email,
  ...
  return rows.map((r) => {
    const sales = r.estimated_monthly_sales != null ? Number(r.estimated_monthly_sales) : null;
